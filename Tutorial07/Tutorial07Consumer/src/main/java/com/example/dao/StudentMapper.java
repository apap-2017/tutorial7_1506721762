...
public interface StudentMapper {
	...
	@Results(value = {
			...
			javaType=List.class,
			many=@Many(select="getStudentsByCourse"))})
	CourseModel selectCourse(@Param("id_course") String id_course);
	
	@Select("select s.name, s.npm from student s, studentcourse sc where s.npm = sc.npm and id_course = #{id_course}")
	List<StudentModel> getStudentsByCourse(@Param("id_course") String id_course);
}
